...
- `publisher`: A tool that takes distributables and "publishes" \(normally just
  uploads\) them somewhere \(for example, GitHub releases\)
- `plugin`: A tool that hooks into Forge's internals and can inject logic into
  your build process
